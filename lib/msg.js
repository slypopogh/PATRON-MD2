const { proto, downloadContentFromMessage, getContentType } = require('baileys-mod')
const fs = require('fs')
...
const sms = (conn, m, store) => {
    ...
    if (m.message) {
	    ...
	    m.quoted.isBaileys = m.quoted.id ? m.quoted.id.startsWith('BAE5') && m.quoted.id.length === 16 : false
			...
			let q = await store.getMessage({
				remoteJid: m.chat,
				id: m.quoted.id,
				fromMe: false,
				participant: m.quoted.sender
			});
			m.quoted.message = q ? (q.message || q) : undefined;
    ...
    if (m && m.msg && m.msg.url) m.download = () => conn.downloadMediaMessage(m.msg);
